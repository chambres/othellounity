...
public class BoardManager : MonoBehaviour
{
    ...
    IEnumerator flip(Vector3 loc, int row, int col)
    {
		...
		int u = affectedMovesX[affectedMovesX.Count-1];
		int q = affectedMovesY[affectedMovesY.Count-1];
		Debug.Log("pos"+u+""+q);
		board[u][q].gameObject.GetComponent<Piece>().setLast = true;

        //changeMousePiece();
        //Debug.Log(String.Join(",", affectedMovesX));

        //for (r = 2.5f; r < 18f; r += 2.25f)
		for(r = 3.348687f; r < 15f; r+=2.21f)
        {
